...
dt2<-subset(dt,select=c("Date","Time","Sub_metering_1","Sub_metering_2","Sub_metering_3","Voltage","Global_reactive_power"),Date %in% c("1/2/2007","2/2/2007"));
dt2$dt<-strptime(paste(dt2$Date,dt2$Time), format="%d/%m/%Y %H:%M:%S");
dt2$Global_active_power<-is.numeric(dt2$Global_active_power)*.002;
dt2$"Global_reactive_power"<-as.numeric(dt2$"Global_reactive_power")*0.002;
dt2$"Voltage"<-as.numeric(dt2$"Voltage")*0.1;
par(mfrow=c(2,2));
#plot(dt2$dt,is.numeric(dt2$Global_active_power),typ='l',ylab="Global Active Power(Kilowatts)",xlab="");
...
#legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),pch=20,col = c("black","red","blue"),width=50);
